...
export default function AdminTagsPage() {
  ...
  const openEditModal = (t: TagItem) => {
    setName(t.name);
    setSlug(t.slug);
    setFormModal({ open: true, editId: t.id });
  };
  ...
  const submit = async () => {
    ...
    try {
      if (formModal.editId) {
        await apiFetch(`/admin/tags/${formModal.editId}`, { method: "PUT", body: JSON.stringify({ name: name.trim(), slug: slug.trim() }) });
        showToast("Đã cập nhật tag", "success");
      } else {
        await apiFetch("/admin/tags", { method: "POST", body: JSON.stringify({ name: name.trim(), slug: slug.trim() }) });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
          <Tag size={24} className="text-indigo-500" /> Quản lý Tags
        </h1>
        <button
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border-brand bg-surface-elevated/50">
                  <th className="w-[35%] px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-text-muted text-left">Tên tag</th>
              ...
              <tbody className="divide-y divide-border-brand">
                {tags.map((t) => (
                  <tr key={t.id} className="hover:bg-surface-elevated transition-colors">
                    <td className="px-5 py-4 align-middle text-left text-sm font-semibold text-text-primary">{t.name}</td>
                    <td className="px-5 py-4 align-middle text-left text-sm text-text-secondary font-mono">{t.slug}</td>
                    ...
                    <td className="px-5 py-4 align-middle text-left">
                      <div className="flex items-center justify-start gap-2">
                        <button onClick={() => openEditModal(t)} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-indigo-500 hover:text-indigo-600 hover:bg-indigo-500/10 transition-all flex items-center gap-1.5 cursor-pointer border border-indigo-500/30">
                          <Edit2 size={13} /> Sửa
                        </button>
                        <button onClick={() => setDeleteModal({ open: true, id: t.id, name: t.name })} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-rose-500 hover:bg-rose-500/10 transition-all flex items-center gap-1.5 cursor-pointer border border-rose-500/30">
                          <Trash2 size={13} /> Xóa
                        </button>
                      </div>
                    </td>
                ...
                {tags.length === 0 && (
                  <tr><td colSpan={4} className="px-5 py-12 text-center text-text-muted"><Tag size={28} className="mx-auto mb-2 opacity-30" /><p className="text-sm">Chưa có tag nào. Thêm tag đầu tiên!</p></td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-text-primary">
              ...
              {formModal.editId ? "Chỉnh sửa tag" : "Thêm tag mới"}
            </h3>

            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-text-secondary mb-1.5">Tên tag</label>
                <input
                  type="text"
      ...
      <ConfirmModal isOpen={deleteModal.open} title="Xóa tag" message={`Bạn có chắc muốn xóa tag "${deleteModal.name}"?`} confirmText="Xóa" cancelText="Hủy" variant="danger" onConfirm={doDelete} onCancel={() => setDeleteModal({ open: false, id: "", name: "" })} />
    </div>
  );
}
